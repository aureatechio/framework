...
;(function () {
  ...
  function runDashboardMain() {
    (function() {
      ...
      const LIGACAO_TIPO_ID = 'a23a700b-673e-4e7f-afed-8f0eb56c1455';
      ...
      // Regra de produto v207: só contabilizar reuniões com score_final preenchido OU do tipo Ligação.
      // Reuniões sem score E que não são ligação NÃO entram nas métricas.
      ...
      let state = {
        ...
        revenueChartSeriesVisible: { Realizado: true, AnoPassado: true, Meta: true, Projecao: true, Pipeline: true },
      ...
      async function fetchMarketingSpend() {
        ...
        try {
          ...
          // fetchSpendTotal: sem filtro de campanha — retorna gasto total da conta (visão global)
          const fetchSpendTotal = async (startYmd, endYmd) => {
            let total = 0;
            const buildUrl = () => {
          ...
          // fetchSpendByCampaignIds: filtra por IDs (usado quando agência está selecionada)
          ...
          // Sem agência selecionada: busca total da conta (sem filtro de campanha)
          // Com agência selecionada: filtra pelas campanhas da agência via timeline_campanhas
          ...
          if (hasAgency) {
            ...
            idsInvest = normalizeIds([...idsLanding, ...idsWhatsapp]);
            if (!idsInvest.length) {
              state.marketingInvestment = 0;
      ...
      async function fetchRevenue() {
        ...
        qPipe = applyCutoffTimestamp(qPipe, 'data_compra').gte('data_compra', chartRange.start).lte('data_compra', chartRange.end);
        if (state.selectedSeller) qPipe = qPipe.eq('vendedoresponsavel', state.selectedSeller);

        // Ano passado: usar UTC puro para alinhar com data_compra histórico (DATE = midnight UTC).
        // chartRange usa timezone local (BRT), que ao shift -1 ano mantém offset +3h e exclui dia 1.
        // Fix: extrair mês/dia LOCAL do chartRange e reconstruir em UTC.
        ...
        const lastYearMonthEnd = (() => {
          const d = new Date(chartRange.end);
        ...
        // Mês anterior: usar UTC para alinhar com data_compra histórico (DATE = midnight UTC)
        ...
        const [
          ...
          monthlyMeta,
          ciclos
        ] = await Promise.all([qCurr, qPrev, qMonth, qPrevMonthMtd, qPipe, pMeta, pCiclos]);

        const rowsPipe = (pipeRows || []).filter(r => !isCancelledPipe(r && r.cancelado));
        ...
        try {
          ...
          // Pipeline cumulative series (mesmos buckets de rawDates)
          try {
          ...
          // Projeção Pipeline (Run Rate)
          try {
            if (revenueMode === 'month' || revenueMode === 'year') {
              ...
              const isPipeYearly = !!chartData.isYearly || revenueMode === 'year';
              let pipeCurrentKey;
              if (isPipeYearly) {
                const now = new Date();
        ...
        // Não filtra por statusReuniao (inclui canceladas). Exclui diretores + usuários internos.
      ...
      async function fetchMeetings() {
        ...
        try {
          ...
          if (a) a.textContent = `Total no período`;
        } catch (e) {}
        ...
        try {
          ...
          // Filtrar: excluir reuniões de diretores + internos
        ...
        if (dataNow) {
            ...
            // "Acontecendo agora" são reuniões em andamento — não exigir score
      ...
      async function fetchMetasData() {
        ...
        try {
          // 1) Sellers (inclui elegivel_rotacao para meta global)
          const { data: sellers } = await sbClient
            .from('vendedores')
          ...
          // 4) Reuniões (agendamento) — conta reuniões com score IA OU tipo Ligação (sem filtro de canceladas)
          ...
          // 5) Meta global = soma das metas individuais (vendedores com elegivel_rotacao)
      ...
      function renderFunnel(data) {
        ...
        const dataCells = data.map((d, idx) => {
          ...
          return `<div class="funnel-data-cell">
            ...
            <div class="funnel-data-label">${escapeHtmlLite(d.l)}${funnelInfoIcon}</div>
          </div>`;
        }).join('');
      ...
      async function fetchFunnelData() {
        ...
        // 4. Reuniões (score IA ou Ligação, sem filtro de canceladas, exclui diretores+internos)
        let directorIds = [];
        try {
          const { data: excluded } = await sbClient
            .from('vendedores')
        ...
        let reunioesWithoutDirectors = (reunioesFiltered || []).filter(r => !directorIds.includes(r.vendedor));
        // Só conta reuniões com score preenchido OU tipo Ligação
        reunioesWithoutDirectors = reunioesWithoutDirectors.filter(isValidMeeting);
      ...
      async function fetchConversionRates() {
        ...
        // Helper: filtrar leadIds para aqueles no período (e seller quando aplicável)
      ...
      async function fetchEtapaNamesByIds(ids) {
        ...
        if (!sbClient || !ids || !ids.length) return result;
        try {
          ...
          console.error('[fetchEtapaNamesByIds] erro:', e);
        }
        return result;
      }
      ...
      async function fetchStageDwellTimes() {
        ...
        try {
          ...
          // Also update global caches (dashboard_tela uses state.sellerNameById / state.sellerImgById)
          try {
            const nameMap = state.sellerNameById || {};
            const imgMap = state.sellerImgById || {};
            (sellersDb || []).forEach(s => {
      ...
      function renderStageDwellTable() {
        ...
        const bodyRows = rows.map((r, rIdx) => {
          ...
          const imgUrl = r.perfilImg || null;
          const avatarInner = imgUrl
            ? `<img src="${String(imgUrl).replace(/"/g, '&quot;')}" alt="${escapeHtmlLite(initials)}" onerror="this.onerror=null;this.parentNode.textContent='${escapeHtmlLite(initials)}';">`
      ...
      async function fetchPipelineData() {
        ...
        try {
          ...
          let meetingsRowsFiltered = await filterRowsByAgencyViaLeadId((meetingsRows || []), (m) => m && m.leadId);
          // Só conta reuniões com score preenchido OU tipo Ligação
          meetingsRowsFiltered = (meetingsRowsFiltered || []).filter(isValidMeeting);
      ...
      function renderRevenue(chartData) {
        ...
        const chartOptions = {
          ...
          colors: (() => {
            ...
            if (hasPipeline) baseColors.push('#f59e0b'); // Pipeline amber
            ...
            if (hasProjPipe) baseColors.push('#d97706'); // Proj. Pipeline dark amber
            if (hasProjecao15d) baseColors.push('#2563eb'); // Projeção 15d
            return baseColors;
          })(),
          // Meta: smooth. Projeção: tracejada. Projeção 15d: tracejada azul.
          stroke: {
            curve: series.map(s => (s && (s.name === 'Projeção' || s.name === 'Projeção 15d' || s.name === 'Proj. Pipeline')) ? 'straight' : 'smooth'),
            width: series.map(() => 2),
            dashArray: series.map(s => {
              if (!s) return 0;
              if (s.name === 'Projeção' || s.name === 'Projeção 15d') return 6;
              if (s.name === 'Proj. Pipeline') return 5;
              if (s.name === 'Pipeline') return 0;
              return 0;
            })
          },
          fill: {
        ...
        if (revenueChart) {
          try {
            ...
            // updateOptions já inclui series no chartOptions — não chamar updateSeries separado (duplica dados)
            ...
            try {
              ...
              if (!(state && state.revenueChartSeriesVisible && state.revenueChartSeriesVisible.AnoPassado)) {
                ...
                const lyName = names.find(n => n && n !== 'Realizado' && n !== 'Meta' && n !== 'Projeção' && n !== 'Projeção 15d' && n !== 'Pipeline' && n !== 'Proj. Pipeline') || null;
        ...
        const getLastYearSeriesName = () => {
          try {
            ...
            return names.find(n => n && n !== 'Realizado' && n !== 'Meta' && n !== 'Projeção' && n !== 'Pipeline' && n !== 'Proj. Pipeline') || null;
          } catch (e) {
            return null;
          }
        };
